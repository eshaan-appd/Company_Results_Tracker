...
# Streamlit UI
# =========================================
st.set_page_config(page_title="Results Tracker", layout="wide")
st.title("📈 Listed Company Results Tracker")
st.caption("Fetch BSE announcements → pick PDF → upload to OpenAI → summarize the results. Summaries are generated by OpenAI models.")
...
def summarize_pdf_with_openai(
    ...
    """
    Uses the Responses API with a file attachment. The model reads the PDF and returns a
    consolidated-results summary in a fixed 5-bullet format with YoY/QoQ and margin math.
    """
    f = _upload_to_openai(pdf_bytes, fname=f"{_slug(company or 'doc')}.pdf")

    # Prompt specialized for CONSOLIDATED results extraction + exact 5-bullet output
    task = f"""
You are an equity analyst. Read the attached BSE financial results PDF for **{company or 'NA'}**.

STRICT SCOPE:
- Use the **CONSOLIDATED** financial statements only (Income Statement / Statement of Profit & Loss, and Balance Sheet).
- Ignore standalone numbers. If consolidated is not disclosed, reply with: "Consolidated statement not disclosed in the PDF."

UNITS & NORMALIZATION:
- Normalize all figures to **INR crore** with 2 decimals (convert if PDF uses lakhs/millions/thousands).
- Show percent changes to 1–2 decimals; show margin delta in **bps** (no sign = positive, prefixed "-" for contraction).

CALCULATIONS:
- Revenue from Operations = as per consolidated P&L.
- EBITDA = Revenue − (Total expenses **before** Depreciation/Amortisation and Finance Costs).
  * If a single "Total Expenses" includes D&A and Finance Costs, back out both to get "expenses before D&A & Finance".
- EBITDA margin (%) = EBITDA / Revenue × 100.
- YoY = vs the same quarter last year (or same period last year for half-year/year).
- QoQ = vs the **immediately preceding quarter** (if current is quarterly; if annual/half-year, set QoQ as "Not disclosed").
- Expense-line commentary: focus on **Cost of Materials** (aka "Cost of materials consumed", "Raw material consumed", etc.)
  and **Employee Benefit Expenses**; report YoY % change for each (if available).
- Finance Costs trend: give YoY % change and the reported value this period.

OUTPUT FORMAT — return EXACTLY these 5 numbered bullets, nothing else:
...
FORMATTING RULES:
- Keep exactly 5 bullets, numbered 1–5 with a dash after the number (e.g., "1- ").
- Use "Not disclosed" where a comparison/value is unavailable (e.g., QoQ for annual results).
- Use minus sign for negatives (e.g., -81.70 Cr, -153.2%).
- Do not include any extra commentary or sections beyond the 5 bullets.

Now read the PDF and produce the five bullets.
"""
